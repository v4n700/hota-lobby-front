...
export class PlayerStatisticsReputationWebMapper
  implements
    Mapper<
      PlayerStatisticsReputationWebEntity[],
      PlayerStatisticsReputationModel[]
    >
{
  mapFrom(
    param: PlayerStatisticsReputationWebEntity[]
  ): PlayerStatisticsReputationModel[] {
    return param.map(
      (item) =>
        ({
        ...
        } as PlayerStatisticsReputationModel)
    );
  }

  mapTo(
    param: PlayerStatisticsReputationModel[]
  ): PlayerStatisticsReputationWebEntity[] {
    return undefined;
  }
}
